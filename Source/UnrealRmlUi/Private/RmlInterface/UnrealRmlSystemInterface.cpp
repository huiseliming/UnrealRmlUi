#include "UnrealRmlSystemInterface.h"
...
CUnrealRmlSystemInterface::CUnrealRmlSystemInterface()
{
	//class FRmlInputProcessor : public IInputProcessor
	//{
	...
	//		SystemInterface->PressedKeySet.Add(InKeyEvent.GetKey());
	//		return false; 
	//	}
	//	virtual bool HandleKeyUpEvent(FSlateApplication& SlateApp, const FKeyEvent& InKeyEvent) 
	//	{
	//		CUnrealRmlSystemInterface* SystemInterface = static_cast<CUnrealRmlSystemInterface*>(Rml::GetSystemInterface());
	//		SystemInterface->PressedKeySet.Remove(InKeyEvent.GetKey());
	//		return false; 
	//	}
	//};
	//FSlateApplication::Get().RegisterInputPreProcessor(MakeShared<FRmlInputProcessor>());
//#if PLATFORM_WINDOWS
//	static class FWindowsMessageHandler
...
bool CUnrealRmlSystemInterface::LogMessage(Rml::Log::Type type, const Rml::String& message)
{
	switch (type)
	{
	case Rml::Log::LT_ERROR:
	case Rml::Log::LT_ASSERT:
		UE_LOG(LogTemp, Error, TEXT("Rml > %s"), UTF8_TO_TCHAR(message.c_str()));
		break;
	case Rml::Log::LT_WARNING:
		UE_LOG(LogTemp, Warning, TEXT("Rml > %s"), UTF8_TO_TCHAR(message.c_str()));
		break;
	default:
		UE_LOG(LogTemp, Log, TEXT("Rml > %s"), UTF8_TO_TCHAR(message.c_str()));
		break;
	}
	return true;
}

void CUnrealRmlSystemInterface::SetMouseCursor(const Rml::String& cursor_name)
{
	if (cursor_name == "move")
	{
		CachedMouseCursorType = EMouseCursor::CardinalCross;
	}
	else if (cursor_name == "pointer")
	{
		CachedMouseCursorType = EMouseCursor::Hand;
	}
	else if (cursor_name == "resize")
	{
		CachedMouseCursorType = EMouseCursor::ResizeSouthEast;
	}
	else if (cursor_name == "cross")
	{
		CachedMouseCursorType = EMouseCursor::Crosshairs;
	}
	else if (cursor_name == "text")
	{
		CachedMouseCursorType = EMouseCursor::TextEditBeam;
	}
	else if (cursor_name == "unavailable")
	{
		CachedMouseCursorType = EMouseCursor::SlashedCircle;
	}
	else
	{
		CachedMouseCursorType = EMouseCursor::Default;
	}
}

void CUnrealRmlSystemInterface::SetClipboardText(const Rml::String& text)
{
	FPlatformApplicationMisc::ClipboardCopy(UTF8_TO_TCHAR(text.c_str()));
}

void CUnrealRmlSystemInterface::GetClipboardText(Rml::String& text)
{
	FString Result;
...
void CUnrealRmlSystemInterface::ActivateKeyboard(Rml::Vector2f caret_position, float line_height)
{
	if (SRmlWidget::FocusedRmlWidget.IsValid())
	{
		CachedCaretPosition = caret_position;
		CachedLineHeight = line_height;
		SRmlWidget::FocusedRmlWidget->ActivateIME();
	}
}

void CUnrealRmlSystemInterface::DeactivateKeyboard()
{
	if (SRmlWidget::FocusedRmlWidget.IsValid())
	{
		SRmlWidget::FocusedRmlWidget->DeactivateIME();
	}
}
